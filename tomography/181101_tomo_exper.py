...
#dirname = "../data/2018/9/25/machida-san_2018_09_25/"
...
idelay =  -26	 #-22#-22 #-21	 #-20, -26 #apply this delay to rf zerocrossing time
...
if not read_rf_waveform:
	...
	freq_a = [1/t for t in tof_a]

	calc_rf_time_offset = False
	if calc_rf_time_offset:
		...
		ffit_coef = np.polyfit(ttime_a[it1:it2], freq_a[it1:it2],1)
		ffit = np.poly1d(ffit_coef)
		print ffit_coef
		print "freq at t=0 from RF waveform ",ffit(0)
		offset =  (fzero_ideal - ffit_coef[1])/ffit_coef[0]
		print "offset [ms]",1e3*offset

		t_rf_table = np.linspace(0, ttime_a[-1], 100)
		#plt.axvline(x = t1)
		#plt.axvline(x = t2)
		plt.axvline(x= offset, color='r')
		plt.axhline(y= fzero_ideal, color='gray')
		#plt.plot(tof_m1_a, 'ko-')
		plt.plot(ttime_a, ffit(ttime_a),'m')
		plt.plot(ttime_a[1:], freq_a, 'b-')

		plt.plot(t_rf_table, [f_rf_lookup(t) for t in t_rf_table],'k-')
		plt.ylim(ymin=1.5e6)
		plt.show()
		sys.exit()
...
for index_time in range(len(tomo_times)):
	...
	if intensity_calc:
		...
		print "intensity maximum at ",ttime_a[imax]
		fig = plt.figure()
	...
	for idelay1 in idelay_a:
		...
		for irc in recon_id:
			...
			print "image_data_a shape ",image_data_a.shape
			#split into array
