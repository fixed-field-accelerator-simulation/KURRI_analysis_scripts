...
import tomo_defs_190401 as tdefs
...
idelay =  -9#-47#-9
#-47 for high res data 28/3/19
#-9 for low res data on 28/3
#-46#-24#-27#-24 #-13 #-23 for flattop 21/9	 #-38 if phi_s is 33 degrees?
...
if calc_rf_time_offset:
	...
	#29/3 time of jump at 0.00221557818824

	#time close to phase jump on 27/3, 28/3, 29/3 = 2.2155e-3 
	tomo_times = np.array([0.18e-3]) #2.16e-3 #after2.215e-3 this is with respect to offset if add_time_offset is True
	
	nturns_sort = 500#10000
...
for index_time in range(len(tomo_times)):
	...
	nslices = int(npfac*points_per_turn)
	...
	calc_mean_max_phase = True
	if calc_mean_max_phase:
		
		deltaph = phase_ax[1] - phase_ax[0]

		max_prof_index = np.argmax(prof_data_a, axis=1)
		phase_profmax = [phase_ax[m] for m in max_prof_index]
		
		mean_phase_data = [sum([p1*f for p1,f in zip(phase_ax, prof)])/sum(prof) for prof in prof_data_a]
		std_phase_data = [np.sqrt(sum([f*(p1-meanp)**2 for p1,f in zip(phase_ax, prof)])) for prof,meanp in zip(prof_data_a,mean_phase_data)]
		#print "phase_profmax ",phase_profmax[10],np.std(prof_data_a[10])

		ia_l = [0, 200]

		print "number of time bins ",nslices, "time interval [ns] ",time_interval_ns

		integrated_profile = np.sum(prof_data_a, axis=0)
		print "len integrated_profile ",len(integrated_profile)
		#plt.plot(phase_ax, integrated_profile)
		#plt.axvline(x=phi_s_deg)
		#plt.show()

		print "binslz+synch_index ", binslz+synch_index, sync_bin
		#print "sum profile, outer part ",sum(prof_data_a[ia]), sum(prof_data_a[ia][:21])+ sum(prof_data_a[ia][-19:])
		col_l = ['k','r','b']
		i1 = 0
		for ia in ia_l:
			print "time [ms], mean phase [deg] ",1e3*ttime_a[ia],mean_phase_data[ia]
			plt.plot(phase_ax, prof_data_a[ia],col_l[i1]+'o-')
			plt.axvline(x=mean_phase_data[ia],linewidth=2,color=col_l[i1],label='<phase>')
			plt.axvline(x=phase_profmax[ia],linewidth=2,color=col_l[i1],linestyle='--',label='phase@f_max')
			i1 = i1 + 1

		
		plt.axvline(x=phase_ax[binslz],color='gray')
		plt.axvline(x=phase_ax[binslz+synch_index],color='gray',linestyle='--')
		plt.axvline(x=phase_ax[nslices - binsuz],color='gray')
		#plt.axvline(x = mean_phase_data[ia] - std_phase_data[ia])
		#plt.axvline(x = mean_phase_data[ia] + std_phase_data[ia])
		plt.legend()
		plt.show()
		
		#plt.subplot(211)		
		plt.plot(mean_phase_data,'k-')
		#plt.plot(std_phase_data,'b--')
		plt.plot(phase_profmax, 'r-')
		...
		#plt.subplot(212)
		#plt.plot(std_phase_data,'b-')
		#plt.plot(np.sum(prof_data_a, axis=1))	
		plt.show()
	...
	if show_mountain_range:
		...
		tdefs.mountain_range(prof_data_a, nturns_sort, nslices, incl_xaxis = True, xaxis= xaxis_deg, xlabel='phase [deg]', extra_data = [mean_phase_data, phase_profmax])			

	if show_imshow:
		tlim1 = 1e3*(ttime_a[0] - time_offset)
		tlim2 = 1e3*(ttime_a[-1] - time_offset)
		#extent = [0, nslices, tlim1, tlim2]
		
		#print "time first/last turn ",ttime_a[0], ttime_a[-1]
		extent = [-180, 180, tlim1, tlim2]
		tdefs.imshow_plot(prof_data_a,extent,ylabel='time',xlabel='arbitrary phase [deg.]', extra_data = [mean_phase_data, phase_profmax])
	...
	if intensity_calc:
		tdefs.intensity_calc_fn(prof_data_a, phase_ax, ttime_a, time_interval_ns, dirname, fname_sig, t_phis_file = t_phis_file, write_intensity_file = write_intensity_file)
		sys.exit()
	...
	for iscan in range(nscan_param):
		...
		tomo_code_param = [nturns, nslices, synch_index, dtbin_phase, binslz, binsuz, recon_start, recon_stop, recon_step, Bnom, Bdot, V_rf, Rnom, rho, gamma_tr]
		...
		print "phase_ax ",phase_ax
		print "phase_tomo_deg ",phase_tomo_deg
...
if animate:
	...
	for irc in recon_id:
		...
		levels = np.linspace(0, cont_limit, 30)
		print "plot phasespace at turn ",irc, ttime_a[irc-1]
		...
		print "mean phase (tomo) ",sum([p1*t1 for p1,t1 in zip(xaxis, tomo_time_proj_a[ipl])])/sum(tomo_time_proj_a[ipl])
		print "mean phase (data) ",sum([p1*t1 for p1,t1 in zip(xaxis, profiles_norm_data_a[irc-1])])/sum(profiles_norm_data_a[irc-1])
