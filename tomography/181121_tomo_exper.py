...
if calc_rf_time_offset:
	...
	tomo_times = np.array([0e-3]) #this is with respect to offset if add_time_offset is True
...
dirname = "../data/2018/9/20/machida-san_pattern/"
...
recon_step_frac = 0.125 #0.1 #reconstruction steps in terms of fraction of sychrotron period
...
idelay =  -22 #-23 for flattop 21/9	 #-22#-22 #-21	 #-20, -26 #apply this delay to rf zerocrossing time
...
for index_time in range(len(tomo_times)):
	...
	if intensity_calc:
		...
		for prof_data, t1 in zip(prof_data_a, ttime_a[:-1]):
			...
			if show_intensity_fwhm_example:
				if t1 > 0.1e-3: 
					print "FWHM points, time ",FWHM, FWHM*time_interval
	...
	for idelay1 in idelay_a:
		...
		for irc in recon_id:
			...
			#plt.subplot(121)
			#plt.imshow(image_data_tr)
			#plt.subplot(122)
			#plt.imshow(image_data_fil)
