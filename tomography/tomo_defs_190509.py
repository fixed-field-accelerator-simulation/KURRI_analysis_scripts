...
def synchrotron_period_vs_amplitude(phase_left, phi_s, a1, a2, phase_sep_max, harmonic, omegarev, eta, n_l, coef_l, ang_l):
	...
	for ph_left, ph_right, H1 in zip(phase_left, phase_right, H_phase):
			...
			if len(n_l) == 1:
				...
				while H0_minus_pot_gen(ph_left, phi_s, a2, n_l, coef_l, ang_l, H1) >= 0:
					ph_left = ph_left + tol
				print H0_minus_pot_gen(ph_left, phi_s, a2, n_l, coef_l, ang_l, H1)
				...
				print H0_minus_pot_gen(ph_left, phi_s, a2, n_l, coef_l, ang_l, H1)
				sys.exit()
				while H0_minus_pot_gen(ph_right, phi_s, a2, n_l, coef_l, ang_l, H1) >= 0:
					ph_left = ph_left - tol
				while H0_minus_pot_gen(ph_right, phi_s, a2, n_l, coef_l, ang_l, H1) < diff_lim:
					ph_left = ph_left + tol
		
			print "ph_left ",ph_left
			sys.exit()
			#synchrotron period
...
def imshow_plot(data,extent,ylabel=None,xlabel=None, extra_data = None):
	...
	if extra_data != None:
		...
		for ex_data in extra_data:
			try:
				plt.plot(ex_data, tax ,col_l[i])
			except:
				plt.plot(ex_data[1], ex_data[0] ,col_l[i])
			i = i + 1
...
def intensity_calc_fn(data, phase_ax, ttime_a, time_interval_ns, dirname, fname_sig, write_intensity_file=False):
	...
	fwhm_raw = []
	i1 = 0
	for prof_data, t1, phmean, phstd, phmax in zip(data, ttime_a[:-1], mean_phase_data, std_phase_data, phase_profmax):
		...
		i1 = i1 + 1
		if write_intensity_file:
			print >> fi, repr(t1), repr(int1), repr(FWHM), repr(phmean),repr(phstd),repr(phmax)
	...
	return intensity_raw, mean_phase_data, phase_profmax, std_phase_data
