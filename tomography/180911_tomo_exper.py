...
turn_points = int(round(tof/time_interval))

#set parameters
phi_s_deg = 20#20
n_slices = turn_points
...
print "number points in bucket ",turn_points*bucket_dur_ns/T_ns
print "number points out of bucket ",turn_points*outbucket_dur_ns/T_ns

bucket_points = int(turn_points*bucket_dur_ns/T_ns)

print "bucket points ",bucket_points

binslowerignore = 4 #add zeroes before profile (earlier time)
...
#pmxi, pmx, pmy, pmi, pmy_av, pmy_std = find_peaks(tdat_chf, data_chf, turn_points, find_hminus, x_low = lower_lim, x_high = upper_lim)

#t_peak_av, peak_av1 = adata.movingaverage(pmx, pmy, win_peak_av)			


print "interval ", turn_points

fixrev_time = True

if fixrev_time:

	_, _, _, pmi_peak, _, _ = find_peaks(tdat_chf, data_chf, turn_points, find_hminus, x_low = lower_lim, x_high = upper_lim, find_maxima = True)
	
	t_baseline = [tdat_chf[i] for i in pmi_peak]
	t_delta = [t_baseline[i] - t_baseline[i-1] for i in range(1, len(t_baseline))] 
	...
	istart =  pmi_peak[0] - 2 #index_start1 - turn_points_half + index_max_win
	tstart = tdat_chf[istart]
	#print "max_in_win ",max_in_win, data_chf[istart]
	

	print "istart, tstart ",istart, tstart

	index_turn_start_l= []	
	for it in range(nbturns):
		index_turn_start = istart + it*turn_points - it
		index_turn_start_l.append(index_turn_start)

	index_turn_list = istart + turn_points*np.arange(nbturns)

	data_win_sig_sel = []
	for it, ist in enumerate(index_turn_start_l):	
		data_win =np.array(data_chf[ist:ist+turn_points])
		data_win_sig =  max(data_win) - data_win
		data_win_sig_sel.append(data_win_sig)
		
		plt.plot(range(1,turn_points+1), data_win_sig + it*0.01,'.-')
		
	plt.axvline(x=binslowerignore)
	plt.axvline(x=turn_points - binsupperignore)
	plt.show()
	...
	nbturns_sel = len(prof_data_a)

	


	f1 = open('kurridata.txt', 'w')
	for yprof in prof_data_a:
		
		yprof_norm = yprof/max(yprof)
		
		print "len yprof ",len(yprof)
		for y in yprof:
			print >>f1,y
	f1.close()
...
#prof_data_l = profile_data(tdat_chf, np.array(data_chf), pmi, turn_points)
