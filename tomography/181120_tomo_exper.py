	...
	if intensity_calc:
		...
		for prof_data, t1 in zip(prof_data_a, ttime_a[:-1]):
			...
			if t1 > 0.1:
				...
				plt.plot(prof_data)
				plt.axvline(x=nearest_below)
				plt.axvline(x=pos_extremum+nearest_above)			
				plt.show()
				sys.exit() 
	...
	for idelay1 in idelay_a:
		...
		for irc in recon_id:
			...
			if filter_isolates:
				...
				image_data_tr  = filter_isolated_cells(image_data_fil, struct=np.array([[0,1,0],[0,1,0],[0,1,0]]))
