...
def exec_pyheadtail(z_bin, modify_bunch = False, z_shift = 0, plot_tracking = False):
...
def freq_analysis(bunch_z_track):
	...
	optimize_func = lambda x: x[0]*np.sin(2*math.pi*x[1]*t+x[2]) + x[3] - z_data
...
if run_pyheadtail:
	nrun = 5
	z_shift_a = np.linspace(0.1, 13, nrun)
	...
	for irun in np.arange(nrun):
		...
		prof_data_a, bunch_z_track = exec_pyheadtail(z_bin, modify_bunch = True, z_shift = z_shift, plot_tracking = True)
