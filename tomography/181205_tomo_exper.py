...
def frequency_analysis(data, Qs, sync_bin):
	...
	plt.plot(phase_m_deg, fftabs_a[indch],'k',label='f1')
	plt.plot(phase_m_deg, fftabs_a[2*indch],'r',label='f2')
	plt.xlabel('phase')
	plt.legend()
	plt.show()
...
if animate:
	...
	for irc in recon_id:
		...
		if versus_time:
			...
			plt.plot(phase_sep_deg - phi_off, dp_sep,'r') #bucket
			plt.plot(phase_sep_deg - phi_off, -dp_sep,'r') #bucket
