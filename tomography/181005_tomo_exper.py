...
def calc_tof(lower_lim, points_per_turn, nturns_fit):
...
def calc_tof_rf_waveform(tdat_rf, sig_rf, nturns_fit):
	zero_cross_ind = []
	t_zero_cross = []
...
def sort_data(istart, tof_fit_a, time_indices, nturns, n_slices, interpolate_data = True, nshift = 0):
...
for index_time in range(len(tomo_times)):
	...
	if not read_rf_waveform:
		...
		tof, time_indices, ttime_a = calc_tof(lower_lim, points_per_turn, nturns_fit)
		istart = time_indices[0]
	else:
		tof_m1, indices_m1, ttime_a_m1 = calc_tof(lower_lim, points_per_turn, nturns_fit)
		tof, time_indices, ttime_a = calc_tof_rf_waveform(tdat_rf, sig_rf, nturns_fit)
		istart = time_indices[0]

		print "tof  method 1 ",tof_m1
		print "tof rf ",tof
	...
	nturns_sort = 1000
	prof_data_a, t_turn_data = sort_data(istart, ttime_a, time_indices, nturns_sort, n_slices, interpolate_data)

	ishift = 0
	if shift_data_min or shift_data_max or shift_data_winmin or shift_data_sym or ishift != 0:
		...
		if max_with_turn:
			maxint_l = []
			for i1 in range(nturns_sync, nturns_sort):
				integt = np.sum(prof_data_a[:i1], axis=0)
				
				maxint = np.argmax(integt)
				maxint_l.append(maxint)

			plt.plot(integt)
			plt.show()

			print "usual imax_col ",imax_col
			print "len maxint_l ",len(maxint_l)
			plt.plot(maxint_l,'k.')
			plt.show()
			sys.exit()
		...
		if shift_data_sym:
			...
			for i, r in enumerate(range_chisq_win_l):
				if r < threshold:
					ithres = i
					break
			isym_sol = isym_list[ithres]
			ishift = isym_sol
			print "converges at turn ",turn_chisq_check[ithres]
			print "shift is ",isym_sol
			
			show_sym_plots = False
			if show_sym_plots:
				plt.subplot(311)
				...
				plt.ylabel('chisq range')
				plt.subplot(313)
				plt.plot(turn_list, isym_list)
				plt.ylabel('shift')
				#plt.plot(data_int)
				#plt.plot(data_integ_shift)
				plt.show()

				indsh = np.arange(ld) + isym_sol
		...
		if show_data_switch and index_time == 0:
			...
			show_data(tind, nturns_show, shift=ishift)
		...
		if ishift != 0:
			...
			prof_data_a, t_turn_data = sort_data(istart, ttime_a, time_indices, nturns_sort, n_slices, interpolate_data, nshift= ishift)
	...
	if animate:
		...
		for irc in recon_id:
			...
			prof_data_norm = prof_data_a[irc-1]/max(prof_data_a[irc-1])
			tomo_t_proj_norm = tomo_time_proj_a[ipl]/max(tomo_time_proj_a[ipl])
