...
npts = 1000
dp_sep, t_sep_ns = tdefs.separatrix(phi_s, eta, T, harmonic, npts, a1, a2)
...
if run_pyheadtail:
	...
	bunch_phase_rad = bunch.z*(-harmonic*2*np.pi/circumference)
	bunch_phase_deg = bunch_phase_rad*180/np.pi
	bunch_t_ini_ns =  1e9*(bunch.z*(-harmonic*T/circumference))

	print "T ",T
	print "max(bunch_phase_deg) ",max(bunch_phase_deg)
	#bunch_t_ini_ns = 1e9*(bunch_phase_deg/(360))*T
	...
	if show_initial_distribution:
		...
		#bunch Hamiltonian
		bunch_H = H_fn(bunch_phase_rad, bunch.dp, phi_s, a1, a2)
		...
		if plot_type1:
			...
			plt.subplot(211)
			counts, xbins, ybins,_ = plt.hist2d(bunch_phase_deg, bunch_dp_ini, bins=60, norm=LogNorm())
			#plt.plot(bunch.z, bunch.dp, 'o')
			#plt.hist2d(bunch_t_ini_ns, bunch_dp_ini, bins=60, norm=LogNorm())
			...
			#plt.plot(bunch_phase_deg, bunch.dp, 'o')
	...
	prof_data_l = []
	bunch_z_l = [bunch.z[0]]
	prof_data_l.append(hist_ini[::-1])
	if track:
		for iturn in np.arange(0, nturns-1, 1):
			...
			if plot_tracking and iturn%plotevery == 0:
				...
				if sel_xaxis == 'phase':
					...
					xaxis = bunch_phase
					binax = z_bin*(-harmonic*2*np.pi/circumference)
					xcont = phph
				elif sel_xaxis == 'z':
					xlabel = 'z [m]'
					xaxis = bunch.z
...
if show_mountain:
	...
	it = 0
	for prof_data in prof_data_norm_a:
		lw = 0.5 - (it / 3000)
...
frequency_analysis(prof_data_a, Qs, synch_index_ideal)
...
if fit_max_turn:
	...
	index_max_turn = np.argmax(prof_data_a, axis=1)
	...
	print "max FFT, period, resolution ",fft_max_index, fft_period, 1/nturns
	nxlim = 2*nturns/200
	plt.plot(fft_max_abs)	
	plt.xlim(0,nxlim)
	plt.show()
	...
	g_mean =  0.5*(max(data) + min(data))
	g_amp =  0.5*(max(data) - min(data))
	g_phase = fft_phase
	g_freq = fft_tune
	...
	optimize_func = lambda x: x[0]*np.sin(2*math.pi*x[1]*t+x[2]) + x[3] - data
	...
	#print "g_mean, g_amp, max(data), min(data) ",g_mean, g_amp, max(index_max_turn), min(index_max_turn)
	print "g_amp ",g_amp
