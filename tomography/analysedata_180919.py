...
def find_peaks_new(xdat, ydat, interval, find_hminus, x_low = None, x_high = None, find_maxima=False):
...
def signal_loss_time(tdat,data, method, cut_off_freq = 0.5e6, threshold_volts = 0.05, sigtonoise = 10, show_data = False, wsize = 100):
    ...
    if method == 'fixed':
        ...
        noise_level = np.std(filter_chf[-wsize:])
...
def signal_loss_time_new(tdat, data, threshold_volts = 0.05, sigtonoise = 10, show_data = False, wsize_half = 50):
    """Signal to noise ratio used to find loss time"""
    
    wsize = 2*wsize_half
    noise_level = np.std(data[-wsize:])
    threshold = sigtonoise*noise_level
    
    for j in range(len(tdat)-wsize):
        test = np.std(data[-wsize-j:-j])
        if test > threshold: 
            index_loss_time = len(tdat) - j - wsize_half
            loss_time = tdat[index_loss_time]
            twin = tdat[index_loss_time + wsize_half] - tdat[index_loss_time - wsize_half]
            break
    ...
    return loss_time, index_loss_time, twin
...
def rf_convert(rfdir):
    ...
    #make a list of frequency values corresponding to a given set of time points
    fittedFfunc=findPfromt(rfdat_t,rfdat_F, order=5) #using RF data  
    
    #plt.plot(rfdat_t, rfdat_F)
    #plt.plot(rfdat_t, fittedFfunc(rfdat_t),'r-')
    #plt.show()
    #sys.exit()
    
    return fittedpfunc, fittedFfunc, rfdat_t, rfdat_F, rfdat_E, rfdat_p
