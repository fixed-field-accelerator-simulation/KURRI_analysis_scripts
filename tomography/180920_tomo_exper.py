...
nbturns = 100
phi_s_deg = 0
#tomo_times = np.array([10.4e-4])
tomo_times = np.array([11e-4])#14e-4

npfac = 1 #interpolate onto time axis with npfac times the number of points as the data
interpolate_data = True #if False use interpolated data


binslowerignore = int(npfac*40) #add zeroes before profile (earlier time)
synch_index = int(npfac*40)
binsupperignore = int(npfac*10) #add zeroes after profile (later time)
...
#upper_lim = lower_lim + nbturns*tof_guess
...
def show_data(lower_lim_l):
	...
	for lower_lim in lower_lim_l:
		...
		plt.axvline(x=1e3*(lower_lim + nbturns*tof_guess), color='r')
...
def sort_data(lower_lim, interpolate_data = True):

	nbturns_fit = 400
	upper_lim_fit = lower_lim + nbturns_fit*tof_guess
	
	find_hminus = False
	_, pmx, _, pmi_peak, _, _ = adata.find_peaks_new(tdat_chf, data_chf, nturn_points, find_hminus, x_low = lower_lim, x_high = upper_lim_fit, find_maxima = True)
	...
	istart =  pmi_peak[0] - 3 #+ 5 #3#index_start1 - nturn_points_half + index_max_win
	...
	if not interpolate_data:
		t_dat_split = np.split(t_dat_all, nbturns_fit)
		sig_dat_split = np.split(sig_dat_all, nbturns_fit)
	...
	it = 0
	data_win_sig_sel = []
	for data_win in sig_dat_split[:nbturns]:
		data_win_sig =  max(data_win) - data_win
		data_win_sig_sel.append(data_win_sig)
		plt.plot(range(1,n_slices+1), data_win_sig + it*0.01,'.-')
		it = it + 1
	...
	return tof_fit, prof_data_a, t_turn_data
...
for index_time in range(len(tomo_times)):
	...
	tof_fit, prof_data_a, t_turn_data = sort_data(lower_lim, interpolate_data)	
	tof = tof_fit
