...
dirname = "../data/2018/9/21/machida_san_foil_escape/"
...
tomo_times = np.array([12e-4, 13e-4])#11e-4 #14e-4
recon_turn = 1

flattop = True
flattop_time = 1.1e-3 #estimate

show_data_switch = False
shift_data = True #shifts data so that minimum is at start
...
#upper_lim = lower_lim + nturns*tof_guess
...
def sort_data(istart, fitdiff_a, interpolate_data = True):
	...
	if not interpolate_data:
		...
		t_dat_split = np.split(t_dat_adj, nturns_fit)
		sig_dat_split = np.split(sig_interp, nturns_fit)
		sig_interp = np.interp(t_dat_adj, t_dat_all, sig_dat_all)
		t_dat_split = np.split(t_dat_adj, nturns_fit)
		sig_dat_split = np.split(sig_interp, nturns_fit)
...
for index_time in range(len(tomo_times)):
	...
	if shift_data:
		...
		if imin_col != 0:
			istart = istart + imin_col
			prof_data_a, t_turn_data = sort_data(istart, fitdiff_a, interpolate_data)
			prof_data_tr = np.transpose(prof_data_a)
			data_integ_1 = [sum(d) for d in prof_data_tr]
			
		plt.plot(data_integ_0,'k')
		plt.plot(data_integ_1,'r')
