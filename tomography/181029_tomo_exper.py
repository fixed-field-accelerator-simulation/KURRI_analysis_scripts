...
import tomo_defs_181029 as tdefs
import math
...
tomo_times = np.array([8e-5]) #start time at which do do tomography #8.0e-4 #first turns [8e-5]
...
idelay =  -22 #-22#-22 #-21	 #-20, -26 #apply this delay to rf zerocrossing time
#if idelay=None, use of the following methods to establish it


idelay_corr_phi_s = True #if True, correct above idelay for nonzero phi_s
...
for index_time in range(len(tomo_times)):
	...
	for prof_data in prof_data_a:
		intensity_raw.append(np.trapz(prof_data))
