...
tomo_times = np.array([9e-4]) #start time at which do do tomography #8.0e-4
...
idelay = None #50 #apply this delay to rf zerocrossing time
...
shift_switches = [idelay != None, shift_data_winmin, shift_data_max, shift_data_min, shift_data_sym]
...
for index_time in range(len(tomo_times)):
	...
	if shift_data_min or shift_data_max or shift_data_winmin or shift_data_sym or idelay != None:
		...
		if idelay != None:
			...
			isym_sol = isym_list[ithres] - (sync_bin - int(0.5*n_slices))
			idelay = isym_sol 
	...
	chisq_sum_l = []
	ncheck = 0 #number of settings to check on either side of ideal

	if ncheck == 0:
		synch_index_l = [synch_index]
	else:
		
		#V_rf_a = np.linspace(0.9*V_rf, 1.1*V_rf, 2*ncheck)
		#synch_index_l = [synch_index]*len(V_rf_a)
		synch_index_l = np.arange(synch_index-int(0.5*ncheck), synch_index + int(0.5*ncheck) + 1)
	
	print "synch_index_l ", synch_index_l


	for synch_index_input in synch_index_l:
		...
		if run_tomo_code:
			...
			input_settings["synchbin"] = synch_index_input #int(0.5*n_slices)
		...
		print "chisq summed over turns ",chisq_sum
	

	if len(synch_index_l) > 1:
		plt.plot(synch_index_l, chisq_sum_l,'ko-')
		plt.xlabel('synch index')
		plt.ylabel('chisq sum')
		plt.axvline(x=synch_index, color='gray')
		plt.savefig('synchindexscan')
		plt.show()
		sys.exit()
