...
def sort_data(istart, tof_fit_a, nturns, n_slices, interpolate_data = True, nshift = 0):
...
for index_time in range(len(tomo_times)):
	...
	if not read_rf_waveform:
		...
		tof, i_peak_l, ttime_a = calc_tof(lower_lim, points_per_turn, nturns_fit)
		istart = i_peak_l[0]
	else:
		tof_m1, indices_m1, ttime_a_m1 = calc_tof(lower_lim, points_per_turn, nturns_fit)
		tof, indices, ttime_a = calc_tof_rf_waveform(tdat_rf, sig_rf, nturns_fit)
		istart = indices[0]
	...
	prof_data_a, t_turn_data = sort_data(istart, ttime_a, nturns_sync, n_slices, interpolate_data)
	...
	if shift_data_min or shift_data_max or shift_data_winmin or ishift != 0:
		...
		if ishift != 0:
			...
			prof_data_a, t_turn_data = sort_data(istart, ttime_a, nturns_sync, n_slices, interpolate_data, nshift= ishift)
			prof_data_tr = np.transpose(prof_data_a)
