...
tomo_times = np.array([0.35e-3]) #this is with respect to offset if add_time_offset is True
...
if calc_rf_time_offset:
	...
	nturns_sort = 2000 #10000 for full intensity file
...
recon_step_frac = 0.24 #reconstruction steps in terms of fraction of sychrotron period
...
idelay =  -23 #-23 for flattop 21/9	 #-22#-22 #-21	 #-20, -26 #apply this delay to rf zerocrossing time
...
#time offset 
#Apply offset to time so that t = 0 corresponds to svk table t=0 
#if "2018/9/20/" in dirname:
#	toffset_rf =  -0.038801293e-3
#elif "2018/9/21/" in dirname:
#	toffset_rf = 0.10296254081e-3
#else:
#	print "RF offset time not determined"
#	toffset_rf = 0.0
	
#print "toffset_rf ",toffset_rf
...
if not read_rf_waveform:
	...
	tof, time_indices, ttime_a = tdefs.calc_tof_rf_waveform(tdat_rf, sig_rf, nturns_fit, istart_guess_l[0])
	istart = time_indices[0]

	if len(ttime_a) < nturns_fit:					
		nturns_fit = len(ttime_a)
	...
	if calc_rf_time_offset:
		...
		#it1 = None
		#it2 = None
		#for i, t in enumerate(ttime_a):
		#	if t > t1 and it1 == None:
		#		it1 = i
		#	if t > t2 and it2 == None:
		#		it2 = i
				
		#if it1 == None or it2 == None:
		#	print "time range ",t1," to ",t2, " [s] not in selected range"
		#	sys.exit()
		...
		npfit = 3
		ffit_coef = np.polyfit(ttime_a[1:], freq_a, npfit)
		...
		#if t1 != None:
		#	plt.axvline(x=offset + t1, color='gray')
		#if t2 != None and t2 < ttime_a[-1]:
		#	plt.axvline(x=offset + t2, color='gray')
		...
		plt.plot(ttime_a, 1e-6*ffit(ttime_a),'r', label='data fit')
...
for index_time in range(len(tomo_times)):
	...
	if Ekin_a[0] != None:
		...
		t_phi_sync_ns = phi_s*(1e9*T)/(2*math.pi) #time at synchronous phase
	...
	if intensity_calc:
		...
		plt.ylim(ymin=0)
		plt.xlim(1e3*ttime_a[0],1e3*ttime_a[-1])
		plt.axvline(x=tphase1_ms, color='r')
		plt.axvline(x=tphase1_ms+tphase2_ms, color='r')
		#plt.axvline(x=0.18)
	...
	for idelay1 in idelay_a:
		...
		for irc in recon_id:
			...
			if image_scaled:
				...
				image_std_l.append(np.std(image_data_a)*intensity_totomo[irc-1])
			else:
				image_std_l.append(np.std(image_data_a))
...
if animate:
	...
	#plt.subplot(211)
	levels = np.linspace(0, 4*max(image_std_l), 30)
