...
if calc_rf_time_offset:
	...
	tomo_times = np.array([0.1e-3]) #this is with respect to offset if add_time_offset is True
...
specify_absolute_turns = True
if specify_absolute_turns:
	turns_absolute = 165#330
	recon_step = 65
...
idelay =  -27#-24 #-13 #-23 for flattop 21/9	 #-22#-22 #-21	 #-20, -26 #apply this delay to rf zerocrossing time
...
def show_mountain_range(data, nt, fmount='mountainrange'):
	...
	ampfac = max_range*70000
	
	#plt.plot(range_data_t)
	#plt.show()

	lw_a = np.linspace(0.8, 0.0, 2*nt)
...
def frequency_analysis(data, Qs, sync_bin):
	...
	energy_scale = np.array([(((beta_turn1[0]**2)*(Etot_turn1[0]))/ ((b**2)*(E)))**0.5 for b, E in zip(beta_turn1, Etot_turn1)])
	...
	Qs_pred = Qs_turn1[0]*energy_scale
	...
	for i in range(n_slices):
		...
		if i == 76:
			print "NAFF tune change ",naffq_phase[0] - naffq_phase[-1]
			naff_pred = naffq_phase[0]*energy_scale
			#plt.plot(naff_turn, Qs_turn1, 'ro-',label='prediction 1')
			plt.plot(naff_turn, naff_pred, 'r-',label='prediction 2')
	...
	Qs2 = Qs_fn(phi_s, 2.5e3, harmonic, beta_a[0], Etot_a[0], eta_a)
...
for index_time in range(len(tomo_times)):
	...
	if shift_data_min or shift_data_max or shift_data_winmin or shift_data_sym or idelay != None:
		...
		if idelay != None:
			...
			for i1 in turn_list:
				...
				if i1 in [turn_list[0], turn_list[500], turn_list[1000], turn_list[1425]]:
	...
	for iscan in range(nscan_param):
		...
		if show_tomo_result:
			...
			profiles_norm_recon_raw = read_data_file(tomo_outdir+'/'+fname_prof)
			...
			if show_profile_out:
				...
				show_mountain_range(profiles_norm_data_a, nturns, fmount='mountainrange_data')


				show_mountain_range(profiles_norm_recon_a, nturns, fmount='mountainrange_recon')
