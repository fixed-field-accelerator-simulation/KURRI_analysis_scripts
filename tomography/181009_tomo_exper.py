...
tomo_times = np.array([13e-4]) #start time at which do do tomography #8.0e-4
...
idelay =  -20 #-21 #apply this delay to rf zerocrossing time
...
for index_time in range(len(tomo_times)):
	...
	chisq_sum_l = []
	ncheck = 10 #number of settings to check on either side of ideal
	...
	if ncheck == 0:
		...
		synch_index_a = np.array(synch_index_l)

		delay_a = idelay - (synch_index_a - sync_bin)
	...
	if len(synch_index_l) > 1:
		...
		plt.plot(delay_a, chisq_sum_l,'ko-')
		plt.xlabel('rf delay')
