...
tomo_times = np.array([7.1e-4]) #start time at which do do tomography #7.0e-4
...
for index_time in range(len(tomo_times)):
	...
	t1 = T_rf_ns*(phi_l2/(2*math.pi)) - outbucket_dur_ns 
	...
	if shift_data_min or shift_data_max or shift_data_winmin:
		...
		print "len(runmean) ",len(runmean)
		#runmean = tdefs.running_mean(data_integ_0, n_slices_active)

		print "runmean ",runmean
		...
		if ishift != 0:
			...
			t_turn_data_ns = 1e9*t_turn_data + t1
