...
if calc_rf_time_offset:
	...
	tomo_times = np.array([1.0e-3]) #this is with respect to offset if add_time_offset is True
	use_file_offset = True
	nturns_sort = 500#10*256
...
idelay =  -22 #-13 #-23 for flattop 21/9	 #-22#-22 #-21	 #-20, -26 #apply this delay to rf zerocrossing time
...
def frequency_analysis(data, Qs, sync_bin):
	...
	Qs_amp_2 = (1/(2**0.5))*Qs*(1 - (phase_m**2/16)*(1+(5/3)*((np.tan(phi_s))**2)))
	...
	plt.plot(phase_deg, Qs_amp)
	plt.plot(phase_deg, Qs_amp_2,'m')
	plt.contour(phase_deg, xr,fftabs_a, levels = clev)
	plt.ylim(fmin, fmax)
	plt.axvline(x=phi_l1*180/math.pi, color='gray',linestyle='--')
	plt.axvline(x=phi_l2*180/math.pi, color='gray',linestyle='--')
	plt.xlabel('phase')
	plt.ylabel('tune')
	plt.savefig('freqa_tosca')
	plt.show()
...
for index_time in range(len(tomo_times)):
	...
	for iscan in range(nscan_param):
		...
		dp_a = (p_a - p0_mev_a[index_time])/p0_mev_a[index_time]
		...
		if nscan_param > 1:
			...
			print "tomography vs intput data chisq ",chisq
