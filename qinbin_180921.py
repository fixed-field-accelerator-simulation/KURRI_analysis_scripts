...
time_cut = 13e-3 #cut data after time_cut
...
sigtonoise = 3 #set threshold above noise to determine final loss point
...
if pos_from_filename:
	for files in ch_files_sel:
		...
		probe =[f[2] for f in fsplit]
		fpos = np.array([f[3] for f in fsplit])
