...
class MyDialog(wx.Dialog):
    def __init__(
       ...
       # input value
        ...
        layout1.Add(self.labl_pc,flag=wx.SHAPED|wx.ALIGN_CENTER)
        layout1.Add(self.text_ctrl_pcsg1,flag=wx.SHAPED|wx.ALIGN_CENTER)
       ...
       # split window by 1: Splitter, 2: sizer
        ...
        if sw == 1:
           ...
           # right panel
            ...
            vv = [0., 1.e-6]
            pp = [0., 1.]
            self.p1 = Matplot(self.sp, tt, vv, pp)
           # left panel
...
#       self.statusbar.SetStatusText("Hola")
        ...
        self.labl = wx.StaticText(self.p2, -1, "# of segments:", pos=(20,10))
        self.segm = wx.TextCtrl(self.p2, -1, "1", style=wx.TE_CENTRE, pos=(20,30))

       # button to show another panel
        b = wx.Button(self.p2, -1, "OK", (20,50))
    ...
    def OnButton(self, evt):
       ...
       # remove file for graphic
        ...
        for iseg in range(0,self.nseg):
            win = MyDialog(self, -1, "time, voltage and phis", size=(450, 120),
           ...
           # Obtain values from another window
            if res == wx.ID_OK:
                ...
                rf = Varik(ph0, dph, vt0, dvt, p0_iseg, t0, t1)
...
class Matplot(wx.Panel):
    def __init__(self, parent, ttlim, vvlim, pplim):
...
#       win = MyFrame(self, -1, "This is a wx.Frame", size=(350, 200),
        win = MyDialog(self, -1, "This is a wx.Frame", size=(450, 120),
                  style = wx.DEFAULT_FRAME_STYLE)
        win.Show(True)
...
class Varik:
    def __init__(self, ph0, dph, vt0, dvt, p0_iseg, t0, t1):
        global filenm
...
# calculate rr with new index k
           ...
           # already at the top momentum
            ...
            freq0 = beta0*clight/(2*math.pi*rr0)
           # end of this segment
            if tabs >= (self.t1 - self.t0):
                print tabs, self.t1 - self.t0, pc0
                self.p1_iseg = pc0
                break
        f_w.close()
        f_data_w1.close()
...
# write comment
        ...
        line = '\'***** Coefficients An for phase and Dn for voltage *****\n'
        f_w.write(line)
        line = 'Tacc'+str(ntable)+' = '+str(self.tmseg[-1]-self.tmseg[0])+'\n'
        f_w.write(line)
        line = 'size = Tacc'+str(ntable)+'*clock'+'\n'
        f_w.write(line)
# frequency curve
        deg = 5
...
#        plt.show()
    ...
    def make_file_begin(self):
        ...
        line = '\'***** Initial blank pattern *****\n'
        f_w.write(line)
        line = 'size = Tini*clock\n'
        f_w.write(line)
        line = '\"tmp_ini.wfm\"=0.\n'
        f_w.write(line)
        f_w.write('\n')

        line = '\'***** Final blank pattern *****\n'
        f_w.write(line)
        line = 'size = Tfin*clock\n'
        f_w.write(line)
        line = '\"tmp_fin.wfm\"=0.\n'
        f_w.write(line)
        f_w.write('\n')
    ...
    def make_file_end(self, ntable):
        self.ntable = ntable
        f_w = open('tmp_all.equ', 'a')
        line = '\'***** Concatinate temp file *****\n'
        f_w.write(line)
        line = '\"tmp1.wfm\"=join(\"tmp_ini.wfm\",\"tmp_acc1.wfm\")\n'
        f_w.write(line)
        for itable in range(2,ntable+1):
            line = '\"tmp'+str(itable)+'.wfm\"=join(\"tmp'+str(itable-1)+'.wfm\",\"tmp_acc'+str(itable)+'.wfm\")\n'
            f_w.write(line)
        line = '\"tmp_all.wfm\"=join(\"tmp'+str(ntable)+'.wfm\",\"tmp_fin.wfm\")\n'
        f_w.write(line)
        f_w.write('\n')

        line = '\'***** Delete temp file *****\n'
        f_w.write(line)
        line = 'delete(\"tmp_ini.wfm\")\n'
        f_w.write(line)
        line = 'delete(\"tmp_fin.wfm\")\n'
        f_w.write(line)
        for itable in range(1,ntable+1):
            line = 'delete(\"tmp_acc'+str(itable)+'.wfm\")\n'
            f_w.write(line)
            line = 'delete(\"tmp'+str(itable)+'.wfm\")\n'
            f_w.write(line)
        f_w.close()
